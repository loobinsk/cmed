...
class Quiz(models.Model):
    ...
    post = models.ForeignKey('medtus.Translation', verbose_name=_("Трансляция"), blank=True, null=True)
...
class Progress(models.Model):
    ...
    score = models.TextField(blank=True, default='', verbose_name=_("Score"))

    overallscore = models.SmallIntegerField(blank=True, default=0, verbose_name=_("OverallScore"))
...
#
    ...
    def update_user_answers(self, question, guess):
        current = json.loads(self.answers)
        current[question.id] = guess
        self.answers = json.dumps(current)
        self.save()


    def update_overallscore(self, percent):
        self.overallscore=percent
        self.save()

    def update_score(self, question, score_to_add=0):
        """
...
#
    #    to_find = re.escape(str(question.category).decode('utf-8', 'ignore')) +\
...
#
    #        new_score = ",".join(
    #            [
    #                str(question.category).decode('utf-8', 'ignore'),
    #                str(updated_score).decode('utf-8', 'ignore'),
    #                str(updated_possible).decode('utf-8', 'ignore'), ""
    #            ])
#
...
#
    ...
    #            [
    #                str(question.category).decode('utf-8', 'ignore'),
    #                str(score_to_add).decode('utf-8', 'ignore'),
    #                str(possible_to_add).decode('utf-8', 'ignore'),
    #                ""
    #            ])
        self.save()
...
class Sitting(models.Model):
    ...
    user = models.ForeignKey('account.MyUser', verbose_name=_("User"))
    ...
    def add_incorrect_question(self, question):
        ...
        self.incorrect_questions += str(question.id).decode('utf-8', 'ignore') + ","
        if self.complete:
            self.add_to_score(-1)
        self.save()
    ...
    def get_questions(self, with_answers=False):
        ...
        if with_answers:
            ...
            for question in questions:
                question.user_answer = user_answers[str(question.id).decode('utf-8', 'ignore')]
