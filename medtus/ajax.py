...
def feedback(request, topic, name, position, contact, text):
    ...
    if message.pk:
        try:
            send_mail(u"Новое сообщение обратной связи", render_to_string('emails/feedback_email.html', message), 'feedback@vrvm.ru', [settings.COORD_EMAIL], fail_silently=True)
