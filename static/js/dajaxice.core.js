var Dajaxice = {
  ...
  account: {
    ...
    updatecombo: function (callback_function, argv, custom_settings) {
      Dajaxice.call('account.updatecombo', 'POST', callback_function, argv, custom_settings);
    }
  },

  lichnie: {
    ajaxrequest: function (callback_function, argv, custom_settings) {
      Dajaxice.call('lichnie.ajaxrequest', 'POST', callback_function, argv, custom_settings);
    },
    GiveMeNewPassword: function (callback_function, argv, custom_settings) {
      Dajaxice.call('lichnie.GiveMeNewPassword', 'POST', callback_function, argv, custom_settings);
    },
    InviteCollegue: function (callback_function, argv, custom_settings) {
      Dajaxice.call('lichnie.InviteCollegue', 'POST', callback_function, argv, custom_settings);
    },
    InviteIntoGroup: function (callback_function, argv, custom_settings) {
      Dajaxice.call('lichnie.InviteIntoGroup', 'POST', callback_function, argv, custom_settings);
    },
    deleteImage: function (callback_function, argv, custom_settings) {
      Dajaxice.call('lichnie.deleteImage', 'POST', callback_function, argv, custom_settings);
    }
  },

  circle: {

    inviteintomycircle: function (callback_function, argv, custom_settings) {
      Dajaxice.call('circle.inviteintomycircle', 'POST', callback_function, argv, custom_settings);
    },
    removefrommycircle: function (callback_function, argv, custom_settings) {
      Dajaxice.call('circle.removefrommycircle', 'POST', callback_function, argv, custom_settings);
    },
    removedialog: function (callback_function, argv, custom_settings) {
      Dajaxice.call('circle.removedialog', 'POST', callback_function, argv, custom_settings);
    },
    confirmmycircle: function (callback_function, argv, custom_settings) {
      Dajaxice.call('circle.confirmmycircle', 'POST', callback_function, argv, custom_settings);
    },
  },

  groups: {
    addgroup: function (callback_function, argv, custom_settings) {
      Dajaxice.call('groups.addgroup', 'POST', callback_function, argv, custom_settings);
    },
  },

  medtus: {
    feedback: function (callback_function, argv, custom_settings) {
      Dajaxice.call('medtus.feedback', 'POST', callback_function, argv, custom_settings);
    }
  },
...
(function () {
  ...
  function o(b) {
    ...
    try {
      ...
      i && (f && c && !c.documentElement && g.getResponseHeader("Content-Type").match(/[^\/]+\/[^\+]+\+xml/)) &&
      (c = new window.ActiveXObject("Microsoft.XMLDOM"), c.async = !1, c.validateOnParse = !1, c.loadXML(f));
    ...
    this._object.onreadystatechange = function () {
      if (!m || d) {
        ...
        if (f._aborted)f.readyState = a.UNSENT; else if (f.readyState == a.DONE) {
          delete f._data;
          l(f);
          i && d && window.detachEvent("onunload",
            k);
          h != f.readyState && j(f);
          h = f.readyState
        }
  ...
  a.prototype.setRequestHeader = function (b, a) {
    ...
    this._headers[b] = a;
    return this._object.setRequestHeader(b,
      a)
  };
